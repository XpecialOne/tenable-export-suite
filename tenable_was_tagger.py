...
2. Parse Excel → list of (asset_uuid, tag_value) rows
   - Normalizes UUIDs whose leading zero was stripped by Excel
...
def normalize_uuid(raw: str) -> str:
    """
    Re-pad a UUID whose first segment had its leading zero(s) stripped by Excel.

    Excel treats UUID strings that start with hex digits as numbers in some
    cell formats, silently dropping leading zeros from the first segment.

    Examples
    --------
    '2d1173-aa14-4b2b-86ba-dc8925607ae4'  →  '002d1173-aa14-4b2b-86ba-dc8925607ae4'
    '4cd4b4-409d-44ed-99e1-a5c85660ec36'  →  '004cd4b4-409d-44ed-99e1-a5c85660ec36'
    '0005594a-1af1-45b5-8ec2-b238f5c9091c' →  unchanged (already 8 chars)
    """
    parts = raw.strip().split("-")
    if len(parts) == 5:
        parts[0] = parts[0].zfill(8)  # pad first segment to exactly 8 hex chars
    return "-".join(parts)
...
class TenableClient:
    ...
    def bulk_assign_tags(self, asset_uuids: list[str], tag_value_uuid: str) -> None:
        ...
        for idx, chunk in enumerate(chunks, start=1):
            ...
            payload = {
                "action": "add",
                "assets": chunk,              # plain UUID strings per API spec
                "tags":   [tag_value_uuid],   # plain UUID string per API spec
            }
...
def load_excel(path: str, logger: logging.Logger) -> list[tuple[str, str]]:
    ...
    Parse the Excel file.
      Column A → asset_uuid  (normalized + validated as UUID)
      Column B → tag_value

    Leading zeros stripped by Excel from the first UUID segment are
    restored automatically via normalize_uuid().

    Rows that are blank or cannot be normalized to a valid UUID are
    ...
    # Normalize UUIDs (restore leading zeros stripped by Excel)
    df["asset_uuid"] = df["asset_uuid"].apply(normalize_uuid)

    # Validate UUID format after normalization
    valid_rows: list[tuple[str, str]] = []
    invalid_count = 0
    for _, row in df.iterrows():
        raw = row["asset_uuid"]
        try:
            # uuid.UUID() also canonicalizes casing and separators
            normalized = str(uuid.UUID(raw))
            valid_rows.append((normalized, row["tag_value"]))
        except ValueError:
            logger.warning(
                "Skipping row — '%s' is not a valid UUID even after normalization.", raw
            )
            invalid_count += 1

    logger.info(
        "Loaded %d valid row(s). Skipped %d blank + %d invalid UUID row(s).",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            ...
            "Leading zeros stripped by Excel from UUID first segments are restored automatically."
        )
    )
