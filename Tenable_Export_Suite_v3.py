...
# Load .env from the same directory as this script (important for Scheduled Tasks)
...
def setup_logging(out_dir: str, ts: str) -> None:
    """Configure logging to a file + console.
    ...
    handlers = [
        RotatingFileHandler(log_file, maxBytes=5 * 1024 * 1024, backupCount=10, encoding="utf-8"),
    ...
    logging.basicConfig(
        ...
        handlers=handlers,
        force=True,  # ensure Scheduled Task / repeated runs don't keep old handlers
    )
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--log-dir",
        default=None,
        help="Directory where log files will be written (default: a 'logs' folder next to the script).",
    )
    parser.add_argument(
        "--disable-was",
        action="store_true",
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    log_dir = args.log_dir or str(SCRIPT_DIR / "logs")
    setup_logging(log_dir, ts)
